...
var MinecraftLauncher = (function () {
    function MinecraftLauncher(gameConf, folder, username, uuid, accessToken, legacy) {
    ...
    MinecraftLauncher.prototype.start = function (progress, out, err) {
        var that = this;
            ...
            request("https://s3.amazonaws.com/Minecraft.Download/indexes/" + that.gameConf.assets + ".json").pipe(fs.createWriteStream("assets/indexes/" + that.gameConf.assets + ".json")).on("finish", function () {
                ...
                that.downloadAssets(that.gameConf.assets, function () {
                    ...
                    java.stdout.on("data", function (data) {
                        global.logger.emit("out", data);
                        console.log("OUT: " + data);
                        out(data);
                    });
                    java.stderr.on("data", function (data) {
                        global.logger.emit("err", data);
    ...
    MinecraftLauncher.prototype.downloadAssets = function (index, cb) {
    ...
    MinecraftLauncher.prototype.generateFolders = function (callback) {
        var folders = [];
        var baseFolder = this.folder;
        for (var i = 0; i < 256; i++)
        ...
        async.each(folders, function (folder, cb) {
            ...
            if (err)
                console.log("Couldn't create folders! ", err);
            callback();
        });
    };
